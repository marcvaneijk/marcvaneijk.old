...
I got in touch with the product team and together with Ashley Travers (Developer) we looked at the environment. The issue was new for him as well. I was still questioning the certificate warning I got from the Hyper-V host the only time it worked. I proposed to change the server authentication setting setting in the auto generated RDP file from warn me to connect and don’t warn me. And that allowed us to successfully perform a Remote Console session based on IP address.
...
Ashley suggested to create a new cert for the Hyper-V host. Hyper-V requires the common name of the certificate set to the FQDN of the Hyper-V host. So we needed the certificate to contain a subject alternate name with the IP address of the host. The certificate requires some key usage properties to be compatible with Hyper-V. These properties can not be created with makecert so our dear friend PowerShell came to the rescue.
...
$thumbprints = @(dir cert:localmachineMy | Where-Object { $_.subject -eq “CN=<FQDN of the Hyper-V host>” } ).thumbprint
 $thumbprints
...
A Remote Console session to a virtual machine, running on the Hyper-V host, is presented with the new certificate. The new certificate contains the IP address as subject alternate name and matches the IP address that was used to initiate the session from the RD Gateway. The tenant is prompted with the security warning asking if he trusts the certificate. The Remote Console session is set up successfully.

While this a workaround for a small lab environment, is not a solid approach for a couple of reasons. For a larger amount of Hyper-V hosts the process would quickly become difficult or even unmanageable. Every Hyper-V hosts requires its own cert with the IP address of that particular host as subject alternate name. If for some reason the Hyper-V host gets a new IP address assigned, a new certificate would also be required. Besides the management aspect a tenant is asked if he trusts the Hyper-V host this is running his virtual machine when he initiated a Remote Console session. Well, off course he trusts the Hyper-V host. Otherwise he would not run his virtual machine on top of it in the first place.

In my previous blog on Windows Azure Pack Console Connect, IIS Rewrite was used to alter the RDP file that was created. Richard Rundle (Senior Program Manager at Microsoft and owner of the Remote Console feature in Windows Azure Pack) pointed out URL Rewrite as a possible approach to add the line authentication level:i:0 to the auto generated RDP file. Now, why didn’t I think of that.
...
<outboundRules>
 ...
 <action type=”Rewrite” value=”negotiate security layer:i:1 authentication level:i:0″ />
 </rule>
 <preConditions>
...
Remote Console with the VMConnectHostIdentificationMode set to IP now allows the tenant to connect successfully, while still complying with a Service Provider security rules. I’d like to thank to Ashley Travers and Richard Rundle for their support.
