...
The hosts in the tenant cluster have the Host Management adapter in VLAN 50 untagged. There are no management virtual machines in the tenant cluster. So VLAN 50 untagged is not required in that host group.
...
Management_Hosts Management Management_Hosts 172.30.50.31-172.30.50.39 172.30.50.254 172.30.50.51

172.30.50.52
 Res.local 
Management_VMs Management Management_VMs 172.30.50.111-172.30.50.150 172.30.50.254 172.30.50.51
...
Complete the Create Logical Network Wizard. For management of network virtualization from VMM it is required that the managements host are assigned an IP address in the PA Space from an IP Pool. Select to create an IP Pool from the Fabric tab, give a descriptive name (in this example Private Address Space) and select the logical network Cloud Network.
...
For isolating tenant or departments workloads, in most environments VLANs are used. To prepare System Center VMM for VLANs we need to define the VLANs in a logical network. We will create one logical network and use network sites within this logical network to define the VLANs.
...
Name Logical Network Network Site IP Range Default Gateway DNS Servers DNS Suffix 
101 Tenants VLANs 101 172.30.101.2-172.30.101.200 172.30.101.1 4.2.2.1

4.2.2.2
  
102 Tenants VLANs 102 172.30.102.2-172.30.102.200 172.30.102.1 4.2.2.1
...
Provider Address Space Cloud Network No Isolation   
...
The VM Networks created on the logical network Cloud Network are used by the logical switch to create a vNIC in the Provider Address Space used for Network Virtualization. In the Create VM Network Wizard for the Provider Address Space select No Isolation to create a direct pass-through to the PA Space. This VM Network will only be used to create vNICs in the PA Space for hosts in the tenant cluster or management VMs that required access to the PA Space from the management cluster.
...
We also need to create one additional virtual port profile that we will use for the Network Virtualization vNIC. The settings used for this vNIC are displayed in the following table .

Name Port Profile Type Offload Settings Security Settings Bandwidth Settings 
PA Space Virtual network adapter Virtual Machine Queues

IPsec task offloading
   

In the Fabric tab select to create a new Hyper-V Port Profile. Give it a descriptive name (in this example PA Space) and select virtual network adapter profile as the type.

30 PA Space

In the offload settings screen select Enable virtual machine queue and Enable IPsec task offloading to configure the settings that would also be enabled by default when you create new virtual machine from Hyper-V Manager.

31 Offload settings

In the Security Settings screen leave all choices unchecked to match the default settings when creating a VM from Hyper-V manager. In the Bandwidth Settings screen leave all bandwidth values to 0.

The virtual network adapter profile will be abstracted by a Port Classification. Select to create a Port Classification from the Fabric tab. Give the Port Classification a descriptive name (in this example PA Space).
