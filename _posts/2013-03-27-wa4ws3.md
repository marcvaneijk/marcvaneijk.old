...
•Installing and configuring Windows Azure for Windows Server 2012 – Part 2
...
When the new tenant selects a plan that was made public, the descriptions we specified earlier are shown. When the new tenant enters an email address, a password and clicks sign up the user account will be created in the SQL database and the tenant logs in to the Tenant Site.

It is possible to create an additional step for a new tenant by specifying an invitation code in the public plan.

02 Invitation code hosting plan
...
06 Change VM Network subnet VMM
...
07 Change VM Network subnet Portal
...
11 VM Custom Create Select VM Network
...
When the Virtual Machine is deployed the tenant is able to make changes to the “hardware settings” of the virtual machine. The tenant can change these settings by applying a different Hardware Profile. The tenant can choose from the Hardware Profiles we have created in System Center VMM 2012 SP1, that are assigned to the hosting plan. A virtual machine needs to be shut down before another Hardware Profile can be applied.

12 Change Hardware configuration
...
A virtual machine needs to be shut down before any of these objects can be attached.
...
In the next part we will look at connecting App Controller to a Service Provider Foundation. But not before I displayed two tweets by two guys who are also working with this product extensively. Experiencing all kind of challenges and questions from Service Providers who are interested in Windows Azure for Windows Server. I’d would like to add TCP Endpoints to their list.

22 Hans Vredevoort23 Kristian Nese
